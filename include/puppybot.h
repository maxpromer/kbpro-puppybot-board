...
void showGraph(){
  ...
  do {
    //tft_.fillRect(20,0,140,128,ST77XX_BLACK);
    for(int i = 0;i<_NumofSensor;i++){
      tft_.setTextColor(ST77XX_WHITE, ST77XX_BLACK);
      tft_.setTextSize(1);
      drawString("A"+String(i)+"=",0,10*i);
      tft_.fillRect(20,10*i,100,5,ST77XX_BLACK);
      tft_.fillRect(20,10*i,ReadLightSensor(i),5,ST77XX_ORANGE);
      drawString(String(ReadLightSensor(i))+"  ",130,10*i);
    }
    tft_.setTextSize(2);
    ...
    if(millis()-lastTimeUpdateBackground >= 100){
      ...
      tft_.setTextColor(flagBackground?ST77XX_RED:ST77XX_GREEN, flagBackground?ST77XX_BLUE:ST77XX_YELLOW);
      drawString("  SW1 Press  ",0,115);
    }
    delay(50);
  } while (digitalRead(6) == 1);
  tft_.fillScreen(ST77XX_BLACK);
...
void setCalibrate(int cal_round) {
  ...
  for (int round_count = 0; round_count < cal_round; round_count ++ ) {
    drawString("Count="+String(cal_round-round_count)+"   ",0,90);
